...
export default function EditCoursePage() {
  ...
  const fetchCourse = async () => {
    try {
      ...
      setFormData({
        ...
        duration: {
          value: course.duration?.value?.toString() || '',
          unit: course.duration?.unit || 'weeks'
        },
        mode: course.mode || 'online',
        schedule: course.schedule || '',
        price: {
          amount: course.price?.amount?.toString() || '',
          currency: course.price?.currency || 'USD',
          isFree: course.price?.isFree || false
        },
        ...
        certification: {
          offered: course.certification?.offered || false,
          name: course.certification?.name || '',
          issuedBy: course.certification?.issuedBy || ''
        },
  ...
  const handleSubmit = async (e: React.FormEvent, newStatus?: string) => {
    ...
    try {
      ...
      await api.put(`/courses/${params.id}`, payload);
          ...
          <form onSubmit={(e) => handleSubmit(e)}>
            ...
            <Card className="mb-6">
              ...
              <p className="text-sm text-gray-600 mb-2">
                Current status: <span className={`font-medium ${formData.status === 'published' ? 'text-green-600' : formData.status === 'draft' ? 'text-yellow-600' : 'text-gray-600'}`}>
